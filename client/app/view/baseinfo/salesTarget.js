    Ext.define('Ysn.view.baseInfo.salesTargetsearch', {
        extend: 'Ext.panel.Panel',
        xtype: 'salesTarget-search',

        requires: [
            'Ysn.view.baseInfo.salesTargetsearchController', 
	        'Ysn.store.*'
        ],

        controller: 'salesTarget-search',
        ...
        id: 'salesTarget-search',
        reference: 'salesTarget-search',
        defaults: {
            layout: 'form',
            xtype: 'container',
                    ...
                    store: {
                        ...
                        listeners: {
                            load: function (store) {
                                ...
                                var bizCd = Ext.getCmp('salesTarget-search').down('#bizGroup');
                                if (store.data.items.length < 2) {
                                    Ext.getCmp('salesTarget-search').down('#user_cd').store.load({ params: { dept_cd: bizCd.getValue(), up_dept_cd: '' } });
                                } else {
                                    Ext.getCmp('salesTarget-search').down('#user_cd').store.load({ params: { dept_cd: '', up_dept_cd: bizCd.getValue() } });
                                }
                            }
                        }
                    },
    ...
    Ext.define('Ysn.view.baseInfo.salesTargetsearchController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.salesTarget-search',
        init: function () {
        ...
        openWindow: function () {
            ...
            hidfield.setValue('salesTarget-search');
            win.down('#cust_nm').setValue(this.lookupReference('cust_nm').getValue());
            Ext.getCmp('baseInfo-salesTarget').add(win);
        ...
        openWindow2: function () {
            ...
            hidfield.setValue('salesTarget-search');
            Ext.getCmp('baseInfo-salesTarget').add(win2);
        ...
        openWindow3: function () {
            ...
            hidfield.setValue('salesTarget-search');
            Ext.getCmp('baseInfo-salesTarget').add(win3);
        ...
        onSubmitClick: function () {
            var pl = Ext.getCmp('salesTargetList');
            ...
            pl.getStore().load(
                    {
                        ...
                        callback: function (records, operation, success) {
                            var col = pl.getController('baseInfo-salesTargetList').changeColumns(operation._params.base_yy);
                            pl.reconfigure(col);
    ...
    Ext.define('Ysn.view.baseInfo.salesTargetList', {
        extend: 'Ext.grid.Panel',
        xtype: 'salesTargetList',
        requires: [
            'Ysn.view.baseInfo.salesTargetListController', 
		    'Ysn.store.salesTargetList',
		    'Ext.grid.filters.Filters'
        ],
    
        controller: 'baseInfo-salesTargetList',     
	    store: {
	        type: 'salesTargetList',
		    autoLoad: false,
            autoDestroy: true
        }, 
    ...
    Ext.define('Ysn.view.baseInfo.salesTargetListController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.baseInfo-salesTargetList',
        init: function () {
            var Today = new Date();
        ...
        ,edit: function (grid, rowIndex, colIndex) {
            if (Ext.getCmp('salesTarget-search').lookupReference('preunit').getValue() != '1') {
            ...
            Ext.Ajax.request({
                url: '/BaseInfo/xlsxBaseSalesUpdate',
                method: 'post',
                params: {
        ...
        var columns = [
            {
                ...
                filter: {
                 ...
                 { text: year+Locale.getMsg('년 합계'), width: 100, dataIndex: 'TOTAL', sortable: true },
                 {
    ...
    Ext.define('Ysn.view.baseInfo.salesTarget', {
        extend: 'Ext.panel.Panel',
        xtype: 'baseInfo-salesTarget',
        requires: [
            'Ysn.view.baseInfo.salesTargetController',
		    'Ysn.view.common.searchcustomer',
            'Ysn.view.baseInfo.salesTargetsearch',
            'Ysn.view.baseInfo.salesTargetList'
        ],

        controller: 'baseInfo-salesTarget',
        reference: 'baseInfo-salesTarget',
        id: 'baseInfo-salesTarget',
        layout: 'border',
        width: 500,
        height: 400,
        overflow: 'hidden',
        ...
        dockedItems: [
	                   {
	                       title: '',
	                       xtype: 'salesTarget-search',
	                       reference: 'salesTargetSearch',
	                       itemId: 'salesTargetSearch',
                           scrollable: true,
	                       collapsible: true,
	                       floatable: true,
	                       split: true,
        ...
        items: [
		    {
		        ...
		        region: 'center',
		        xtype: 'salesTargetList',
		        reference: 'salesTargetList',
		        itemId: 'salesTargetList',
		        listeners: {
		            //select: 'itemclick'
		        }
    ...
    Ext.define('Ysn.view.baseInfo.salesTargetController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.baseInfo-salesTarget',
        requires: [
	        'Ysn.store.*'
        ],
        init: function () {
            this.lookupReference('salesTargetSearch').setTitle(pageTitle); 
        },
        openWindow: function () {
            /* if (this.lookupReference('salesTargetSearch').lookupReference('unit').getValue() != '1') {
        ...
        firstFormSave: function () {
            ...
            var pl = this.lookupReference('salesTargetSearch');

            if (form.isValid()) {
                form.submit({
                    url: '/BaseInfo/xlsxBaseSalesUpload',
                    waitMsg: 'Uploading .',
                    success: function (fp, o) {
                        ...
                        pl.getController('salesTarget-search').onSubmitClick();
        ...
        xlsExport: function () {
            ...
            Ext.getCmp('salesTargetList').saveDocumentAs({
			headerRowCnt: 1,
                type: 'xlsx',
                title: Ext.getCmp('salesTarget-search').getTitle(),
                fileName: Locale.getMsg('매출목표')
            });
        },
        xlsFrmDown: function () {   
            var link = document.createElement("a");
            document.body.appendChild(link);
            link.download = '매출목표등록양식.xlsx';
            link.href = 'resources/' + encodeURIComponent('매출목표등록양식.xlsx');
            link.click();
        }
    });
