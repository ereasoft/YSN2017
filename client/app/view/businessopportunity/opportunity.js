Ext.define('Ysn.view.businessopportunity.opportunitysearch', {
    extend: 'Ext.panel.Panel',
    xtype: 'opportunity-search',

    requires: [
        'Ysn.view.businessopportunity.opportunitysearchController',
	  	'Ysn.view.common.searchcustomer',
	    'Ysn.store.*'
    ],

    controller: 'opportunity-search',
                ...
                store: {
                    ...
                    listeners: {
                        load: function (store) {
                            ...
                            var bizCd = Ext.getCmp('opportunity-search').down('#bizGroup');
                            if (store.data.items.length < 2) {
                                Ext.getCmp('opportunity-search').down('#user_cd').store.load({ params: { dept_cd: bizCd.getValue(), up_dept_cd: '' } });
                            } else {
                                Ext.getCmp('opportunity-search').down('#user_cd').store.load({ params: { dept_cd: '', up_dept_cd: bizCd.getValue() } });
                            }
                        }
                    }
                },
...
Ext.define('Ysn.view.businessopportunity.opportunitysearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.opportunity-search',
    init: function () {
    ...
    openWindow: function () {
        ...
        hidfield.setValue('opportunity-search');
		win.down('#cust_nm').setValue(this.lookupReference('cust_nm').getValue());
        Ext.getCmp('businessopportunity-opportunity').add(win);
    ...
    openWindow2: function () {
        ...
        hidfield.setValue('opportunity-search');
        Ext.getCmp('businessopportunity-opportunity').add(win2);
    ...
    openWindow3: function () {
        ...
        Ext.getCmp('businessopportunity-opportunity').add(win3);
    ...
    onSubmitClick: function () {
        ...
        Ext.getStore('opportunityList').load(
                {
                    params: {
...
Ext.define('Ysn.view.businessopportunity.opportunityList', {
    extend: 'Ext.grid.Panel',
    xtype: 'opportunityList',
    requires: [
        'Ysn.view.businessopportunity.opportunityListController', 
		'Ysn.store.opportunityList',
		'Ext.grid.filters.Filters' 
    ],
    
    controller: 'businessopportunity-opportunityList',     
	store: {
	...
	columns: [
			...
			{ text: Locale.getMsg('수주예정일'), width: 125, dataIndex: 'WSO_PDATE', renderer: Ext.util.Format.dateRenderer('Y-m-d'), sortable: true, filter: true },
			{
			...
			{text: "악성업체유무", width: 125, dataIndex: 'BAD_AR_YN', sortable: true, hidden: true, 
			  filter: {
				type: 'string',
				itemDefaults: {
					emptyText: Locale.getMsg('검색어입력..')
...
Ext.define('Ysn.view.businessopportunity.opportunityListController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.businessopportunity-opportunityList',
    init: function () {
...
Ext.define('Ysn.view.businessopportunity.opportunityDetail',{
    extend: 'Ext.form.Panel',
    xtype: 'businessopportunity-opportunityDetail',
    requires: [
        'Ysn.view.businessopportunity.opportunityDetailController'
    ],

    controller: 'businessopportunity-opportunityDetail', 


    frame: true,
    id: 'opportunityDetail',
	reference: 'opportunityDetail',
    bodyPadding: 10,
    scrollable:true,
    width: 900,   
	fieldDefaults: { 
    ...
    items: [{
        ...
        items: [
                    ...
                    {
                        fieldLabel: Locale.getMsg('사업유형'), xtype: 'textfield', labelWidth: 80, name: 'biz_type',
                        xtype: 'combobox',
                        reference: 'biz_type',
                        publishes: 'value',
                        displayField: 'CODE_NM',
                    ...
                    {
                        fieldLabel: Locale.getMsg('유통구조'), xtype: 'textfield', labelWidth: 100, name: 'dstr_type',
                        xtype: 'combobox',
                        reference: 'dstr_type',
                        publishes: 'value',  
                        displayField: 'CODE_NM',
        ...
        items: [                   
					{
					    ...
					    itemId: 'opportunityItem',
					    reference: 'opportunityItem',
					    columnLines: true,  
					    columns: [
        ...
        dockedItems: [{
            ...
            items: [
                ...
                { xtype: 'component', flex: 1 },
                 { xtype: 'button', text: Locale.getMsg('재발주초기화'), name: 'btn1', itemId: 'btn1',margin: '5 5 5 5', hidden: false,
                     listeners: {click : 'onRepeat'}
                 },
				{ xtype: 'button', text: Locale.getMsg('DROP'), name: 'btn2', itemId: 'btn2',margin: '5 5 5 5', hidden: false,
                    listeners: {click : 'onDrop'}
				},             
				{ xtype: 'button', text: Locale.getMsg('저장'), name: 'btn3', itemId: 'btn3',margin: '5 5 5 5', hidden: false,
...
Ext.define('Ysn.view.businessopportunity.opportunityDetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.businessopportunity-opportunityDetail',
    init: function () {
        this.lookupReference('wso_psblt').store.load({ params: { up_code_id: 'WSO_PSBLT',lang:localeCd } });
        this.lookupReference('oppt_status').store.load({ params: { up_code_id: 'OPPT_STATUS', lang: localeCd } });
        this.lookupReference('biz_type').store.load({ params: { up_code_id: 'BIZ_TYPE', lang: localeCd } });
    ...
    openWindow: function () {
        ...
        var hidfield = win.query('#paentFrm')[0];
        hidfield.setValue('opportunityDetail');
        win.down('#item').setValue(this.lookupReference('item_nm').getValue());
        Ext.getCmp('opportunityDetail').add(win);
        win.show();
        //}
    },
    resetVal: function () {
    ...
    openWindow2: function () { 
        ...
        hidfield.setValue('opportunityDetail');
        Ext.getCmp('opportunityDetail').add(win3);
        win3.show(); 
    },
    resetVal2: function () {
        this.lookupReference('cust_nm').setValue('');
    ...
    openWindow3: function () {
        ...
        hidfield.setValue('opportunityDetail');
        Ext.getCmp('opportunityDetail').add(win4);
        win4.show();
    },
    resetVal3: function () {
        this.lookupReference('euser_nm').setValue('');
    ...
    onDrop: function () {
        var dropPop = Ext.getCmp('common-opptdropPop');
        var Frm = Ext.getCmp('opportunityDetail').getForm();
        var oppt_cd = Frm.findField('oppt_cd').getValue(); 
        if (!dropPop) {
            dropPop = new Ysn.view.common.opptdropPop();
        }
        var hidfield = dropPop.query('#parentfrm')[0];
        hidfield.setValue('opportunityDetail');
        dropPop.lookupReference('common-opptdropPopDetail').getForm().load({
            url: '/Opportunity/opportunityDetail?oppt_cd='+oppt_cd,
            success: function (form, action) {
                var dataVal = Ext.JSON.decode(action.response.responseText);
               dropPop.lookupReference('common-opptdropPopDetail').down('#btn1').show();
            }
        })
        Ext.getCmp('opportunityDetail').add(dropPop);
        dropPop.show();
    },
    ...
    onRepeat: function () {
        var pl = Ext.getCmp('opportunityDetail');
        var form = pl.getForm();
        var data = pl.lookupReference('itemList').down('#opportunityItem').getStore().getRange();
        form.findField('oppt_cd').setValue('');
        form.findField('oppt_type').setValue('OPTP_200');
        form.findField('oppt_type_nm').setValue('REPEAT');
        form.findField('oppt_sumry').setValue('');
        form.findField('wso_pdate').setValue('');

        for (var i = 0; i < data.length; i++) {
            data[i].set('SALES_DATE', '');
            pl.down('#oppt_sales_date' + i).setValue('');
        }
        Ext.Msg.alert(Locale.getMsg('재발주초기화완료'), Locale.getMsg('납품예정정보(수량.단가)및 수주예정일 입력후 저장버튼을 클릭하세요.'));
    },

    onSubmit: function () {
		var pl = Ext.getCmp('opportunityDetail');
        var form = pl.getForm();
        Ysn.Util.cbEmptyVal(Ext.getCmp('opportunityDetail'));
        if (form.findField('oppt_type').getValue() == 'OPTP_200') {
            var data = pl.lookupReference('itemList').down('#opportunityItem').getStore().getRange();
            for (var i = 0; i < data.length; i++) {
                if (data[i].get('SALES_DATE') == '' || data[i].get('OPPT_QTY') == '' || data[i].get('OPPT_UNIT_PRC') == '') {
                    Ext.Msg.alert('Warning', '납품예정정보의 수량,단가,납품예정일은 필수 입력입니다.');
                    return false;
                } 
            }
        }
        if (form.isValid()) {
            var url = '/Opportunity/opportunitySave';
...
Ext.define('Ysn.view.businessopportunity.opportunity', {
    extend: 'Ext.panel.Panel',
    xtype: 'businessopportunity-opportunity',
    requires: [
        'Ysn.view.businessopportunity.opportunityController',
		'Ysn.view.common.searchcustomer'
    ],

    controller: 'businessopportunity-opportunity',
    reference: 'businessopportunity-opportunity',
    id: 'businessopportunity-opportunity',
    layout: 'border',
    width: 500,
    height: 400,
    overflow: 'hidden',
    ...
    items: [
		{
		    ...
		    region: 'center',
		    xtype: 'opportunityList',
		    itemId: 'opportunityList',
		    listeners: {
		        select: 'itemclick'
		    }

		}, {
		    title: Locale.getMsg('사업기회 관리'),
...
Ext.define('Ysn.view.businessopportunity.opportunityController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.businessopportunity-opportunity',
    requires: [
	    'Ysn.store.*'
    ],
    init: function () {
        this.lookupReference('opportunitySearch').setTitle(pageTitle);
        if (Ext.getCmp('chkpopup').getValue()) {
            Ext.getCmp('businessopportunity-opportunity').down('#east').setVisible(false);
        }
        this.lookupReference('muiltFile1').down('#paentFrm').setValue('opportunityDetail');
        ...
        this.lookupReference('opportunityDetail').down('#muiltFileBox1').down('#paentFrm').setValue('businessopportunity-opportunity');
        this.lookupReference('opportunityDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
    },
    itemclick: function (view, record, index, e) {
        var Pl = Ext.getCmp('opportunityDetail');
        var Frm = Pl.getForm();
        if (Ext.getCmp('chkpopup').getValue()) {
            openPopupView(Locale.getMsg('사업기회 관리'), 'businessopportunity-opportunityDetail', record.get('OPPT_CD'), 'opportunity|opportunityView|oppt_cd');
        } else {
            Frm.reset();
            Ext.each(Pl.query('hiddenfield[name=oppt_item_cd]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.each(Pl.query('hiddenfield[name=oppt_sales_date]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.each(Pl.query('hiddenfield[name=oppt_oppt_qty]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.each(Pl.query('hiddenfield[name=oppt_oppt_unit_prc]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.each(Pl.query('hiddenfield[name=oppt_amount]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.each(Pl.query('hiddenfield[name=oppt_krw_amount]'), function (obj) {
                Pl.remove(obj);
            });
            Ext.getStore('opportunityView').removeAll();
            Pl.down('#muiltFileBox1').down('#AttachFileList').getStore().removeAll();
            Ext.getCmp('businessopportunity-opportunity').lookupReference('muiltFile1').down('#paentFrm').setValue('opportunityDetail');
            Ext.getCmp('businessopportunity-opportunity').lookupReference('muiltFile1').down('#childFrm').setValue('#muiltFileBox1');
            Ext.getCmp('businessopportunity-opportunity').lookupReference('muiltFile1').down('#doc_mgt').setValue('file');
            Ext.getCmp('businessopportunity-opportunity').lookupReference('opportunityDetail').down('#muiltFileBox1').down('#paentFrm').setValue('businessopportunity-opportunity');
            Ext.getCmp('businessopportunity-opportunity').lookupReference('opportunityDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
            Ext.getCmp('businessopportunity-opportunity').mask("loading...");
            Ext.getStore('opportunityView').load({
                params: { oppt_cd: record.get('OPPT_CD') }
                , callback: function (records, operation, success) {
                    setTimeout(function () {
                        ...
                        Pl.items.items[2].body.dom.scrollTop = 0;
                        Pl.items.items[2].body.dom.scrollLeft = 0;
                    }, 500);
                }
			});
			Pl.down('#muiltFileBox1').down('#AttachFileList').getStore().load({
			    params: {
			        biz_gubun: record.get('OPPT_CD'),
			        doc_mgt: 'file',
			        chasu: '0'
			    }
			});
			...
			if (Ext.getCmp('businessopportunity-opportunity').down('#east').collapsed) task.delay(1000);
			var task2 = new Ext.util.DelayedTask(function () {
			    Ext.getCmp('businessopportunity-opportunity').unmask();
			});
			task2.delay(1050);
    ...
    xlsExport: function () {
        ...
        Ext.getCmp('opportunityList').saveDocumentAs({
			headerRowCnt: 1,
            type: 'xlsx',
            title: Ext.getCmp('opportunity-search').getTitle(),
            fileName: Locale.getMsg('사업기회 관리')
        });
    }
});
