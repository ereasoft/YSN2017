Ext.define('Ysn.view.monitoring.msalesReportOrdersearch', {
    extend: 'Ext.panel.Panel',
    xtype: 'msalesReportOrder-search',

    requires: [
        'Ysn.view.monitoring.msalesReportOrdersearchController' 
    ],

    controller: 'msalesReportOrder-search',
    ...
    items: [
        {
            ...
            xtype: 'combobox',
            reference: 'sales_ym_type',
            itemId: 'sales_ym_type',
            publishes: 'value',
            fieldLabel: '당월/누계/년간',
            labelAlign: 'right',
            displayField: 'name',
...
Ext.define('Ysn.view.monitoring.msalesReportOrdersearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.msalesReportOrder-search',
    init: function () {
        var Today = new Date();
        ...
        this.lookupReference('sales_ym_type').setValue(this.lookupReference('sales_ym_type').getStore().getAt(0).get('code'));
    ...
    onSearch: function () {
        var el = this.lookupReference('sales_ym_type').getValue();
        var pl = Ext.getCmp('monitoringmsalesReportOrder');
        switch (el) {
            case '누계':
        ...
        this.lookupReference('month')
        Ext.getStore('msalesReportOrder').load(
            {
                params: {
                    bizGroup: this.lookupReference('bizGroup').getValue(),
                    deptGroup: this.lookupReference('deptGroup').getValue(), 
                    sales_ym_type: this.lookupReference('sales_ym_type').getValue(),
...
Ext.define('Ysn.store.mdetailPop', {
    extend: 'Ext.data.Store',
    alias: 'store.mdetailPop',
    storeId: 'mdetailPop',
    fields: ['CODE_NM', 'VALUE_1','REPORT2_VALUE1_1', 'REPORT2_VALUE1_2', 'REPORT2_VALUE1_3', 'REPORT2_VALUE1_4', 'REPORT2_VALUE1_5', 'REPORT2_VALUE1_6', 'REPORT2_VALUE1_7', 'REPORT2_VALUE1_8', 'REPORT2_VALUE1_9'],
    proxy: { 
        type: 'memory', 
        reader: {
            type: 'json',
...
Ext.define('Ysn.store.msalesReportOrder', {
    extend: 'Ext.data.Store',
    alias: 'store.msalesReportOrder',
    storeId: 'msalesReportOrder',
    fields: ['LIST1', 'MBOCHART', 'LIST2_1', 'LIST2_2', 'LIST3', 'CHART1', 'CHART2', 'CHART3', 'CHART4'],
    proxy: {
        method: "POST",
        type: 'ajax',
        url: '/Monitoring/salesReportOrder',
        reader: {
            type: 'json',
            rootProperty: ''
        }
    ...
    listeners: {
        load: function (store, records, successful, operation) {
            ...
            Ext.getStore('mdetailPop').loadRawData(store.data.items[0].data['LIST4']);
            
            var rec2 = store.data.items[0].data['LIST2_1'];
            td = pl.lookupReference('bDetail');
            td.removeAll();
            var subtotal1 = new Array(0, 0, 0);
            var subtotal2 = new Array(0, 0, 0);
            var total = new Array(0, 0, 0);
            td.add({ text: Locale.getMsg('구분'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
            td.add({ text: Locale.getMsg('유통구조'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
            td.add({ text: Locale.getMsg('목표(A)'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
            td.add({ text: Locale.getMsg('실적/추정(B)'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
            td.add({ text: Locale.getMsg('증감(B-A)'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
            td.add({ text: Locale.getMsg('달성율'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
...
Ext.define('Ysn.view.monitoring.msalesReportOrder', {
    extend: 'Ext.panel.Panel',
    xtype: 'monitoring-msalesReportOrder',
    requires: [
        'Ysn.view.monitoring.msalesReportOrderController',
		'Ext.chart.series.*',
		'Ext.chart.theme.Muted'
    ],

    controller: 'monitoring-msalesReportOrder',
    reference: 'monitoringmsalesReportOrder',
    id: 'monitoringmsalesReportOrder',
    overflow: 'hidden',
    scrollable: false,
    header: false,
    bodyBorder: true,
    ...
    dockedItems: [
	               {
	                   header: false,
	                   xtype: 'msalesReportOrder-search',
	                   reference: 'msalesReportOrderSearch',
	                   collapsible: true,
	                   floatable: true,
	                   split: true,
	                   dock: 'top',
    ...
    items: [
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            tools: [
					          { iconCls: 'x-fa fa-table', handler: 'detail_tb' }
                            ],
                            scrollable: true,
                            reference: 'bDetail',
                            margin: '20 0 0 0',
...
Ext.define('Ysn.view.monitoring.msalesReportOrderController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.monitoring-msalesReportOrder',
    requires: [
    ],
    init: function () {
        this.lookupReference('msalesReportOrderSearch').getController('msalesReportOrder-search').onSearch();
    },
    detail_tb: function () {
        var win  = {
            xtype: 'window',
        ...
        if (!this.lookupReference('detailPop')){
            this.getView().add(win);
        }
        var td = this.lookupReference('detailPop');
        var rec2 = Ext.getStore('mdetailPop').data.items;
       // var td = .lookupReference('detailPop'); 
        var subtotal1 = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        var subtotal2 = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        var total = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        td.add({ text: Locale.getMsg('구분'), colspan: 2, tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('매출목표'), rowspan: 2, tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('당월수주현황'), colspan: 4, tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('당월매출현황'), colspan: 4, tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('팀'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('유통구조'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('이월수주'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('순수수주'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('합계'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('달성율'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('이월수주'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('순수수주'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('합계'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ text: Locale.getMsg('진척율'), tdAttrs: { style: { 'font-weight': 'bold', height: '30px', backgroundColor: '#EFEFEF' } } });
        td.add({ xtype: 'label', text: Locale.getMsg('국영'), rowspan: 7 });
        var ditem = new Array(9);
        var ditem0 = 0, ditem1 = 0, ditem2 = 0, ditem3 = 0, ditem4 = 0, ditem5 = 0, ditem6 = 0, ditem7 = 0, ditem8 = 0, k = 0;
        for (var i = 0; i < rec2.length; i++) {
            td.add({ xtype: 'label', text: rec2[i].get('CODE_NM') });
            for (var j = 0; j < 9; j++) {
                ditem[j] = Ysn.Util.YWnumberFm2(rec2[i].get('REPORT2_VALUE1_'+(j+1)), 1);
                if(j== 4 || j == 8) ditem[j] = ditem[j] + '%';
            } 

            if (i < 6) {
                for (var j = 0; j < 9; j++) {
                    td.add({ xtype: 'label', text: ditem[j] });
                    if (j != 4 && j != 8) subtotal1[j] = subtotal1[j] + parseFloat(ditem[j]);
                }
                if (i == 5) {
                    td.add({ xtype: 'label', text: 'TOTAL', tdAttrs: { style: { 'font-weight': 'bold' } } });
                    for (var j = 0; j < 9; j++) {
                        if (j != 4 && j != 8) td.add({ xtype: 'label', text: Ysn.Util.YWnumberFm2(subtotal1[j], 1), tdAttrs: { style: { 'font-weight': 'bold' } } });
                        if (j == 4) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(subtotal1[3], subtotal1[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
                        if (j == 8) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(subtotal1[7], subtotal1[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
                    }
                    td.add({ xtype: 'label', text: Locale.getMsg('해영'), rowspan: 6 });
                }
            } else {
                for (var j = 0; j < 9; j++) {
                    td.add({ xtype: 'label', text: ditem[j] });
                    if (j != 4 && j != 8) subtotal2[j] = subtotal2[j] + parseFloat(ditem[j]);
                }
            }
        }
        td.add({ xtype: 'label', text: 'TOTAL', tdAttrs: { style: { 'font-weight': 'bold' } } });
        for (var j = 0; j < 9; j++) {
            if (j != 4 && j != 8) td.add({ xtype: 'label', text: Ysn.Util.YWnumberFm2(subtotal2[j], 1), tdAttrs: { style: { 'font-weight': 'bold' } } });
            if (j == 4) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(subtotal2[3], subtotal2[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
            if (j == 8) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(subtotal2[7], subtotal2[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
        }
        td.add({ xtype: 'label', text: 'TOTAL', colspan: 2, tdAttrs: { style: { 'font-weight': 'bold' } } });
        for (var j = 0; j < 9; j++) {
            if (j != 4 && j != 8) total[j] = subtotal1[j] + subtotal2[j];
        }
        for (var j = 0; j < 9; j++) {
            if (j != 4 && j != 8) td.add({ xtype: 'label', text: Ysn.Util.YWnumberFm2(total[j], 1), tdAttrs: { style: { 'font-weight': 'bold' } } });
            if (j == 4) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(total[3], total[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
            if (j == 8) td.add({ xtype: 'label', text: Ysn.Util.YWpercentFm2(total[7], total[0]) + '%', tdAttrs: { style: { 'font-weight': 'bold' } } });
        }
       
        this.lookupReference('detailPop').show();
    },
