...
Ext.define('Ysn.view.samplerequest.requestController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-request',
	requires: [ 
	    'Ysn.store.*'
    ],
    init: function() {
        this.lookupReference('requestSearch').setTitle(pageTitle);
		if(Ext.getCmp('chkpopup').getValue()){ 
			Ext.getCmp('samplerequest-request').down('#east').setVisible(false);			
		}


		Ext.getCmp('requestDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('requestDetail').down('#muiltFileBox2').down('#AttachBtn').hide();

		this.lookupReference('muiltFile1').down('#paentFrm').setValue('requestDetail');
		...
		this.lookupReference('requestDetail').down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-request');
		this.lookupReference('requestDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
		this.lookupReference('muiltFile2').down('#paentFrm').setValue('requestDetail');
		...
		this.lookupReference('requestDetail').down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-request');
		this.lookupReference('requestDetail').down('#muiltFileBox2').down('#childFrm').setValue('#muiltFile2');


	},
	itemclick: function(view, record, index, e) {         
		if(Ext.getCmp('chkpopup').getValue()){
            openPopupView('샘플요청','samplerequest-requestDetail',record.get('SMR_CD'),'SampleRequest|sampleRequestView|smr_cd|smr_chasu|'+record.get('SMR_CHASU')); 
		}else{
            /*Ext.getCmp('requestDetail').down('#btn1').show();
			Ext.getCmp('requestDetail').down('#btn2').show();
			Ext.getCmp('requestDetail').down('#btn3').show();*/
			//if (Ext.getCmp('samplerequest-request').down('#east').collapsed) Ext.getCmp('samplerequest-request').down('#east').toggleCollapse(); 
			Ext.getCmp('requestDetail').load({
				...
				success: function(form, action) {
					...
					if (reqDetail.SMP_STATUS == '' || reqDetail.SMP_STATUS == 'SMSTAT_010' || reqDetail.SMP_STATUS == 'SMSTAT_030') {
					    pl.down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').show();
					    pl.down('#muiltFileBox1').down('#AttachBtn').show();
					} else {
					    pl.down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').hide();
					    pl.down('#muiltFileBox1').down('#AttachBtn').hide();
		...
		Ext.getCmp('requestDetail').getForm().findField('addchk').setValue('modify');

		var task = new Ext.util.DelayedTask(function () {
		    Ext.getCmp('samplerequest-request').down('#east').toggleCollapse();
		});
		if (Ext.getCmp('samplerequest-request').down('#east').collapsed) task.delay(1000);
	...
	frmClear: function(){
	    Ext.getCmp('requestDetail').getForm().reset();
	    Ext.getCmp('requestDetail').getForm().findField('smr_type').setValue('SMRTP_100');
	    Ext.getCmp('requestDetail').getForm().findField('packing_info').setValue('');
	    Ext.getCmp('requestDetail').getForm().findField('project_poss').setValue('A');
	    Ext.getCmp('requestDetail').down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').show();
	    Ext.getCmp('requestDetail').down('#muiltFileBox1').down('#AttachBtn').show(); 
		Ext.getCmp('requestDetail').down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-request');
		Ext.getCmp('requestDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
		Ext.getCmp('requestDetail').down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-request');
		Ext.getCmp('requestDetail').down('#muiltFileBox2').down('#childFrm').setValue('#muiltFile2');
		//Ext.getCmp('requestDetail').getForm().findField('addchk').setValue('add');
		if(Ext.getCmp('samplerequest-request').down('#east').collapsed){
		Ext.getCmp('samplerequest-request').down('#east').toggleCollapse();
		}
		Ext.getCmp('requestDetail').down('#fbtn1').hide(); 
		Ext.getCmp('requestDetail').down('#fbtn2').hide(); 
		Ext.getCmp('requestDetail').down('#fbtn3').hide(); 
		Ext.getCmp('requestDetail').down('#fbtn4').hide(); 
		Ext.getCmp('requestDetail').down('#fbtn5').hide(); 
		Ext.getCmp('requestDetail').down('#btn4').show(); 
	...
	xlsExport: function(){  
		...
		Ext.getCmp('requestList').saveDocumentAs({ 
			headerRowCnt: 1,
            type:       'csv',
            title:      Ext.getCmp('request-search').getTitle(),
            fileName:   '샘플요청' 
		});
	} 
});

Ext.define('Ysn.view.samplerequest.requestsearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.request-search',
    init: function () {
         
         
      //  this.lookupReference('deptLabel').setFieldLabel(Ysn.Util.findLabel('매출조직'));
         
	     this.lookupReference('bizGroup').store.load(); 
		 this.lookupReference('bizGroup').setValue(Ysn.Util.chkDept(true));
		 this.lookupReference('smp_status').store.load({ params: { up_code_id: 'SMP_STATUS',lang: localeCd, mode:'R' } });
	...
	openWindow: function(){
		...
		hidfield.setValue('request-search');
		Ext.getCmp('samplerequest-request').add(win);
	...
	onSubmitClick: function() {
			...
			Ext.getCmp('requestDetail').down('#fbtn4').hide(); 
			Ext.getCmp('requestDetail').down('#fbtn5').hide(); 
			Ext.getStore('sampleRequestList').load(
