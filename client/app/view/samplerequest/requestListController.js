Ext.define('Ysn.view.samplerequest.requestListController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-requestList', 
    init: function() {
	  
	}
});

Ext.define('Ysn.view.samplerequest.requestDetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-requestDetail', 
    init: function() {
	 this.lookupReference('btn4').show(); 
	 this.lookupReference('project_poss').store.load({ params: { up_code_id: 'PROJECT_POSS', lang: localeCd } });
	 this.lookupReference('project_poss').setValue('A');
	 ...
	 this.lookupReference('packing_info').setValue('');
	 ...
	 var pumpLv2 = ['SMRPN_110','SMRPN_120','SMRPN_130','SMRPN_140','SMRPN_150','SMRPN_160','SMRPN_170','SMRPN_180','SMRPN_190','SMRPN_200','SMRPN_210','SMRPN_220','SMRPN_230'];

	 for (var i=0;i<13 ; i++)
	 {  
		pumpItem.insert(idx,{xtype: 'hiddenfield', name: 'pump_id', itemId: 'pump_id'+i, value: pumpLv2[i] });
	  ...
	  pumpItem.insert(idx+2,{ xtype: 'textfield', colspan: 7, name: 'pump_pkg',itemId: 'pump_pkg',inputWrapCls: '', triggerWrapCls: '',fieldStyle: 'background:none',readOnly:true, style:{width:'100%'}}); 
	 ...
	 var bottleLv2 = ['SMRPN_310','SMRPN_320','SMRPN_330','SMRPN_340','SMRPN_350','SMRPN_360'];
	 
	 idx = 8;
	 for (var i=0;i<6 ; i++)
	 {  		
		 
		bottleItem.insert(idx,{xtype: 'hiddenfield', name: 'bottle_id', itemId: 'bottle_id'+i, value: bottleLv2[i]});
	  ...
	  bottleItem.insert(idx+2,{ xtype: 'textfield', colspan: 7, name: 'bottle_pkg',itemId:'bottle_pkg',inputWrapCls: '', triggerWrapCls: '',fieldStyle: 'background:none',readOnly:true, style:{width:'100%'}});       
	 ...
	 var overcapLv2 = ['SMRPN_410','SMRPN_420','SMRPN_430'];
	 idx = 8;
	 for (var i=0;i<3 ; i++)
	 {  
		overcapItem.insert(idx,{xtype: 'hiddenfield', name: 'overcap_id', itemId: 'overcap_id'+i, value: overcapLv2[i]});
	  ...
	  overcapItem.insert(idx+2,{ xtype: 'textfield', colspan: 7, name: 'overcap_pkg',itemId:'overcap_pkg',inputWrapCls: '', triggerWrapCls: '',fieldStyle: 'background:none',readOnly:true, style:{width:'100%'}});    
	...
	onChange: function( cb , newValue , oldValue , eOpts ){
		if(newValue == 'SMRTP_200'){
			Ext.getCmp('requestDetail').lookupReference('sample_prodinfo').show();
			Ext.getCmp('requestDetail').lookupReference('sample_pump').hide();
			Ext.getCmp('requestDetail').lookupReference('sample_bottle').hide();
			Ext.getCmp('requestDetail').lookupReference('sample_overcap').hide();
		}else if(newValue == 'SMRTP_300'){
			Ext.getCmp('requestDetail').lookupReference('sample_prodinfo').show();
			Ext.getCmp('requestDetail').lookupReference('sample_pump').show();
			Ext.getCmp('requestDetail').lookupReference('sample_bottle').hide();
			Ext.getCmp('requestDetail').lookupReference('sample_overcap').show();
		}else{
			Ext.getCmp('requestDetail').lookupReference('sample_prodinfo').hide();
			Ext.getCmp('requestDetail').lookupReference('sample_pump').show();
			Ext.getCmp('requestDetail').lookupReference('sample_bottle').show();
			Ext.getCmp('requestDetail').lookupReference('sample_overcap').show();
		}
	},
	 
	onStatusChg: function (el, newValue, oldValue, eOpts) {
	    var Pl = Ext.getCmp('requestDetail');
	    Pl.down('#old_status').setValue(oldValue);
	    if (newValue == '') {
	        Pl.down('#btn4').setText(Locale.getMsg('임시저장'));
	        Pl.down('#btn4').show(); //저장
	        Pl.down('#btn1').hide(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').hide(); //양산
	        Pl.down('#btn6').hide(); //드롭
	        Pl.down('#btn2').hide(); //삭제
	        Pl.down('#btn5').hide(); //샘플재작업
	        Pl.down('#btn3').hide(); //신규저장
	        Pl.down('#print').hide(); //인쇄
	        Pl.down('#btn10').hide(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    }
	    if (newValue == 'SMSTAT_010') {
	        Pl.down('#btn4').setText(Locale.getMsg('임시저장'));
	        Pl.down('#btn4').show(); //저장
	        Pl.down('#btn1').show(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').hide(); //양산
	        Pl.down('#btn6').hide(); //드롭
	        Pl.down('#btn2').show(); //삭제
	        Pl.down('#btn5').hide(); //샘플재작업
	        Pl.down('#btn3').show(); //신규저장
	        Pl.down('#print').show(); //인쇄
	        Pl.down('#btn10').hide(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    } else if (newValue == 'SMSTAT_020') {
	        Pl.down('#btn4').setText(Locale.getMsg('수정'));
	        Pl.down('#btn4').show(); //저장
	        Pl.down('#btn1').hide(); //샘플요청
	        Pl.down('#btn9').show(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').hide(); //양산
	        Pl.down('#btn6').hide(); //드롭
	        Pl.down('#btn2').show(); //삭제
	        ...
	        Pl.down('#btn10').hide(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    } else if (newValue == 'SMSTAT_030') {
	        Pl.down('#btn4').hide(); //저장
	        Pl.down('#btn1').show(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').hide(); //양산
	        ...
	        Pl.down('#btn3').hide(); //신규저장
	        Pl.down('#print').show(); //인쇄
	        Pl.down('#btn10').show(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    }  else if (newValue == 'SMSTAT_050') {
	        Pl.down('#btn4').hide(); //저장
	        Pl.down('#btn1').hide(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        ...
	        Pl.down('#btn10').show(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    }  else if (newValue == 'SMSTAT_060') {
	        Pl.down('#btn4').hide(); //저장
	        Pl.down('#btn1').show(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').hide(); //양산
	        Pl.down('#btn6').show(); //드롭
	        Pl.down('#btn2').hide(); //삭제
	        Pl.down('#btn5').hide(); //샘플재작업
	        Pl.down('#btn3').hide(); //신규저장
	        Pl.down('#print').show(); //인쇄
	        ...
	        Pl.down('#btn10').show(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	    } else if (newValue == 'SMSTAT_650') {
	        Pl.down('#btn4').hide(); //저장
	        Pl.down('#btn1').hide(); //샘플요청
	        Pl.down('#btn9').hide(); //요청철회
	        Pl.down('#btn8').hide(); //샘플수령 
	        Pl.down('#btn7').show(); //양산
	        Pl.down('#btn6').show(); //드롭
	        Pl.down('#btn2').hide(); //삭제
	        Pl.down('#btn5').show(); //샘플재작업
	        Pl.down('#btn3').hide(); //신규저장
	        Pl.down('#print').show(); //인쇄
	        Pl.down('#btn10').show(); //수정
	        Pl.down('#btn11').hide(); //메일발송
	...
	openWindow: function(){
		//if(!Ext.getCmp('common-searchPartUser')){
			...
			hidfield.setValue('requestDetail');
			Ext.getCmp('requestDetail').add(win);
	...
	,openWindow2: function(){
		//if(!Ext.getCmp('common-searchPartUser')){
			var win2 = Ext.getCmp('commonsearchItem');
			if(!win2){
			  win2 = new Ysn.view.common.searchItem();
			}
			var hidfield = win2.query('#paentFrm')[0];
		    win2.down('#item').setValue(this.lookupReference('item_nm').getValue());
			hidfield.setValue('requestDetail');
			Ext.getCmp('samplerequest-request').add(win2); 
			win2.show();
		//}
	},
	onPPS: function () {
	    var form = Ext.getCmp('requestDetail').getForm();
	    var chkYn = form.findField('spec_yn').getValue();
	        ...
	        var smr_cd = this.lookupReference('smr_cd').getValue();
	        var smr_chasu = this.lookupReference('smr_chasu').getValue();
	        hidfield.setValue('requestDetail');
	        smpCd.setValue(this.lookupReference('smp_cd').getValue());
	        smpChasu.setValue(this.lookupReference('smp_chasu').getValue());
	        mode.setValue(this.lookupReference('smr_type').getValue());
	        Ext.each(frm.query('field[name^=a_]'), function (obj) {
	            obj.setReadOnly(true);
	        });
	        Ext.each(frm.query('field[name^=p_]'), function (obj) {
	            obj.setReadOnly(false); 
	        });
			switch (this.lookupReference('smr_type').getValue())
			{
			case 'SMRTP_100' : 
				frm.items.items[0].hide();
			    frm.items.items[1].show();
				break;
			case 'SMRTP_200' : 
				frm.items.items[0].show();
			    frm.items.items[1].hide();
				break;
			default :
				frm.items.items[0].show();
			    frm.items.items[1].show();
	        ...
	        if (this.lookupReference('specchk_cnt').getValue() != '0') {
	            frm.getForm().load({
	                ...
	                success: function (form, action) {
	                    ...
	                    form.findField('smr_cd').setValue(smr_cd);
	                    form.findField('smr_chasu').setValue(smr_chasu);
	                    if (chkYn == 'Y')  win3.lookupReference('commonspecChkform').down('#btn1').hide();
	                }
	            })
	        }
	        Ext.getCmp('samplerequest-request').add(win3);
    ...
    openMail: function () {
        ...
        Ext.getCmp('samplerequest-request').add(win5);
        win5.show();
        //}
    },
    ...
    onRepairNew: function () {
        if (this.getView().down('#req_repair_yn').getValue() == 'Y') {
            this.getView().down('#btn_yn').setValue('N');
        } else {
    ...
    onRepair: function () {
        ...
        var Frm = Ext.getCmp('requestDetail').getForm();
        ...
        Ext.getCmp('requestDetail').add(repairPop);
        repairPop.show();
    },
    onDrop: function () {
        ...
        Ext.getCmp('requestDetail').add(dropPop);
        dropPop.show();
    }, 
	onProduction: function(){  //팝업
	    var Frm = Ext.getCmp('requestDetail').getForm();
	    Frm.findField('status').setValue('prdt_yn');
		this.onSubmit();
	...
	onReceipt: function () {
	    var Frm = Ext.getCmp('requestDetail').getForm();
	    if (Frm.findField('spec_yn').getValue() == 'Y'){
	        if (Frm.isValid()) {  
	            this.onPPS();
	        }
	    } else {
	        Frm.findField('smp_status').setValue('SMSTAT_650');
	        this.onSubmit();
	    }
	    //

	},
	onRequest: function () {  //팝업
	    var Frm = Ext.getCmp('requestDetail').getForm();

	    /*if (Frm.findField('item_cd').getValue() == '') {
	        Ext.Msg.alert('Warning', Locale.getMsg('제품을 선택하세요'));
	        return false;
	    }*/
		Frm.findField('smp_status').setValue('SMSTAT_020');
	    Frm.findField('smr_rqdate').setValue(new Date());
	    Frm.findField('status').setValue('SMPREQ');
	    this.onSubmit();

	},
	onCancel: function () {
	    var Frm = Ext.getCmp('requestDetail').getForm();
	    Frm.findField('smp_status').setValue('SMSTAT_030');
	    this.onSubmit();

	},
	onDelete: function () {
	    var Frm = Ext.getCmp('requestDetail').getForm();
	    Frm.findField('status').setValue('use_yn');
	    this.onSubmit();
	...
	onReturn: function () {
	    ...
	    Ext.getCmp('requestDetail').add(rejectPop);
	    rejectPop.show();
	},
	onReturn2: function () {
	    ...
	    Ext.getCmp('requestDetail').add(reviewPop);
	    reviewPop.show();
	},
	onCopy: function () {
		Ext.Msg.confirm('신규저장', '데이터를 신규저장하시겠습니까', function (button) {
                    if (button == 'yes'){
						    var pl = Ext.getCmp('requestDetail');
                            var Frm = pl.getForm();
							Frm.findField('smr_cd').setValue('');
							Frm.findField('smr_chasu').setValue('');
							pl.getController().onSubmit();
                    }  
            });
	...
	onModify: function () {
	    var form = Ext.getCmp('requestDetail').getForm();
	    Ysn.Util.cbEmptyVal(Ext.getCmp('requestDetail'));
	    if (form.isValid()) {
	        ...
	        form.submit({
	            ...
	            success: function (form, action) {
	                ...
	                Ext.getCmp('requestDetail').store.reload();
	            },
	...
	onSubmit: function(){ 
		...
		if (status == '' && pl.down('#btn4').getText() == Locale.getMsg('수정')) form.findField('status').setValue('modify');
		Ysn.Util.cbEmptyVal(Ext.getCmp('requestDetail'));
		if (form.isValid()) {
			...
			form.submit({
				...
				success: function(form, action) {
                    ...
                    if (status == 'use_yn') {
                        ...
                        Ext.getCmp('samplerequest-request').down('#east').toggleCollapse()
                    }else 
					{
                       form.reset();         
		                pl.down('#muiltFileBox1').down('#AttachFileList').getStore().removeAll();
					...
					form.findField('smr_type').setValue('SMRTP_100');
					form.findField('packing_info').setValue('');
					form.findField('project_poss').setValue('A');
				    pl.down('#fbtn1').hide(); 
					pl.down('#fbtn2').hide();  
					pl.down('#fbtn3').hide();  
					pl.down('#fbtn4').hide();  
					pl.down('#fbtn5').hide();  
					pl.down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-request');
					pl.down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
					pl.down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-request');
					pl.down('#muiltFileBox2').down('#childFrm').setValue('#muiltFile2');	
					
					Ext.getCmp('request-search').getController('request-search').onSubmitClick();
				    Ext.Msg.alert('처리상태', '처리완료'); 
				},
				failure: function(form, action) {
				    //console.log('response:'+ action);
				    form.findField('smp_status').setValue(form.findField('old_status').getValue());
				    var dataVal = Ext.JSON.decode(action.response.responseText);
				    if (!Ysn.Util.OnsessOut(action.response.responseText)) return false;
					Ext.Msg.alert('Warning', '서비스에 이상이 있으니 관리자에게 문의하세요.'); 
					//Ext.getCmp('keymanList').store.reload();
