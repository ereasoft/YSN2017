...
Ext.define('Ysn.view.samplerequest.receiptController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-receipt',
	requires: [ 
	    'Ysn.store.*'
    ],
    init: function() {
        this.lookupReference('receiptSearch').setTitle(pageTitle);
		if(Ext.getCmp('chkpopup').getValue()){ 
			Ext.getCmp('samplerequest-receipt').down('#east').setVisible(false);			
		} 

		Ext.getCmp('receiptDetail').down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('receiptDetail').down('#muiltFileBox1').down('#AttachBtn').hide();

		this.lookupReference('muiltFile1').down('#paentFrm').setValue('receiptDetail');
		...
		this.lookupReference('receiptDetail').down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-receipt');
		this.lookupReference('receiptDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
		this.lookupReference('muiltFile2').down('#paentFrm').setValue('receiptDetail');
		...
		this.lookupReference('receiptDetail').down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-receipt');
		this.lookupReference('receiptDetail').down('#muiltFileBox2').down('#childFrm').setValue('#muiltFile2');
	},
	itemclick: function(view, record, index, e) { 
		if(Ext.getCmp('chkpopup').getValue()){
            openPopupView('샘플접수','samplerequest-receiptDetail',record.get('SMR_CD'),'SampleRequest|sampleRequestView|smr_cd|smr_chasu|'+record.get('SMR_CHASU')); 
		} else {
		    if (Ext.getCmp('receiptDetail').down('#smp_status').getValue() == 'SMSTAT_020' || Ext.getCmp('receiptDetail').down('#smp_status').getValue() == 'SMSTAT_080') {
		        Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').show();
		        Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachBtn').show();
		    } else {
		        Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').hide();
		        Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachBtn').hide();

		    }
          //  if (Ext.getCmp('samplerequest-receipt').down('#east').collapsed) Ext.getCmp('samplerequest-receipt').down('#east').toggleCollapse(); 
			Ext.getCmp('receiptDetail').load({
				...
				success: function(form, action) {
					...
					var pl = Ext.getCmp('receiptDetail')
					...
					if (reqDetail.SMP_STATUS == 'SMSTAT_020' || reqDetail.SMP_STATUS == 'SMSTAT_080') {
		                    Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').show();
		                    Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachBtn').show();
		                } else {
		                    Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').hide();
		...
		Ext.getCmp('receiptDetail').getForm().findField('addchk').setValue('modify');

		var task = new Ext.util.DelayedTask(function () {
		    Ext.getCmp('samplerequest-receipt').down('#east').toggleCollapse();
		});
		if (Ext.getCmp('samplerequest-receipt').down('#east').collapsed) task.delay(1000);
	...
	frmClear: function(){
	    Ext.getCmp('receiptDetail').getForm().reset(); 
		Ext.getCmp('receiptDetail').down('#muiltFileBox1').down('#AttachFileList').getStore().removeAll();
		Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').getStore().removeAll();
	    Ext.getCmp('receiptDetail').down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-receipt');
	    Ext.getCmp('receiptDetail').down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
	    Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-receipt');
	    Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#childFrm').setValue('#muiltFile2');
		Ext.getCmp('receiptDetail').getForm().findField('addchk').setValue('add');
		Ext.getCmp('receiptDetail').down('#east').toggleCollapse();
	},
	xlsExport: function(){  

		Ext.getCmp('receiptList').saveDocumentAs({ 
			headerRowCnt: 1,
            type:       'xlsx',
            title:      Ext.getCmp('receipt-search').getTitle(),
            fileName:   '샘플접수' 
		});
	} 
});

Ext.define('Ysn.view.samplerequest.receiptsearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.receipt-search',
	init: function() {
     ...
     this.lookupReference('smp_status').store.load({params:{up_code_id:'SMP_STATUS', lang: localeCd ,mode:'A'}});  
	...
	openWindow: function(){
		...
		hidfield.setValue('receipt-search');
		Ext.getCmp('samplerequest-receipt').add(win);
	...
	onSubmitClick: function() {
			...
			Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').getStore().removeAll();
		    Ext.getCmp('receiptDetail').getForm().findField('addchk').setValue('add');
			Ext.getCmp('receiptDetail').down('#fbtn1').hide(); 
			Ext.getCmp('receiptDetail').down('#fbtn2').hide(); 
			Ext.getCmp('receiptDetail').down('#fbtn3').hide(); 
			Ext.getCmp('receiptDetail').down('#fbtn4').hide(); 
			Ext.getCmp('receiptDetail').down('#fbtn5').hide(); 
			Ext.getStore('sampleReceiptList').load(
					{params: {   
