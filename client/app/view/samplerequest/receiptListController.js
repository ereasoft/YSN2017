Ext.define('Ysn.view.samplerequest.receiptListController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-receiptList', 
    init: function() {
	  
	}
});

Ext.define('Ysn.view.samplerequest.receiptDetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.samplerequest-receiptDetail', 
    init: function() {
	 ...
	 //this.lookupReference('smr_type').setValue('');
	...
	onChange: function( cb , newValue , oldValue , eOpts ){
		if(newValue == 'SMRTP_200'){
			Ext.getCmp('receiptDetail').lookupReference('sample_prodinfo').show();
			Ext.getCmp('receiptDetail').lookupReference('sample_pump').hide();
			Ext.getCmp('receiptDetail').lookupReference('sample_bottle').hide();
			Ext.getCmp('receiptDetail').lookupReference('sample_overcap').hide();
		}else if(newValue == 'SMRTP_300'){
			Ext.getCmp('receiptDetail').lookupReference('sample_prodinfo').show();
			Ext.getCmp('receiptDetail').lookupReference('sample_pump').show();
			Ext.getCmp('receiptDetail').lookupReference('sample_bottle').hide();
			Ext.getCmp('receiptDetail').lookupReference('sample_overcap').show();
		}else{
			Ext.getCmp('receiptDetail').lookupReference('sample_prodinfo').hide();
			Ext.getCmp('receiptDetail').lookupReference('sample_pump').show();
			Ext.getCmp('receiptDetail').lookupReference('sample_bottle').show();
			Ext.getCmp('receiptDetail').lookupReference('sample_overcap').show();
		}
	},
    itemAdd: function(){
	...
	itemRemove: function(){
	   ...
	   //this.lookupReference('inq_item_grp').updateLayout();
		...
		//if(!Ext.getCmp('common-searchPartUser')){
			...
			hidfield.setValue('receiptDetail');
			Ext.getCmp('receiptDetail').add(win);
			win.setPosition(70,100);
			win.show();
		//}
	},
	onPPS: function () {
	    var form = Ext.getCmp('receiptDetail').getForm();
	    var chkYn = form.findField('spec_yn').getValue(); 
	        ...
	        Ext.getCmp('samplerequest-receipt').add(win3);
    ...
    openMail: function () {
        ...
        Ext.getCmp('samplerequest-receipt').add(win5);
        win5.show();
        //}
    },
	onStatusChg: function (el, newValue, oldValue, eOpts) {
	    ...
	    if (newValue == 'SMSTAT_020') {
	        Pl.down('#btn1').show(); //요청반려 
	        Pl.down('#btn2').show(); //요청접수 
	        Pl.down('#print').show(); //인쇄
	        Pl.down('#btn11').hide(); //메일발송
	...
	onRepair: function () {
	    ...
	    var Frm = Ext.getCmp('receiptDetail').getForm();
	    ...
	    Ext.getCmp('receiptDetail').add(repairPop);
	    repairPop.show();
	},
	onDrop: function () {
	    ...
	    Ext.getCmp('receiptDetail').add(dropPop);
	    dropPop.show();
	},
	onReturn: function(){
		...
		var btn_yn = Frm.findField('btn_yn').getValue();
			...
			Ext.getCmp('receiptDetail').add(rejectPop); 
			rejectPop.show();
	},
	onReturn2: function () {
	    ...
	    Ext.getCmp('receiptDetail').add(reviewPop);
	    reviewPop.show();
	},
	print: function () {
	    var pl = this.getView();
	...
	onSubmit2: function(){ 
	    var form = Ext.getCmp('receiptDetail').getForm();
	    var sbYn = 'Y';
	    if (!form.findField('file_name')) sbYn = 'N';
	    if (Ext.getCmp('receiptDetail').down('#muiltFileBox2').down('#AttachFileList').getStore().data.items.length > 0) sbYn = 'Y';
	    if (sbYn == 'N') {
				Ext.Msg.alert('파일첨부확인', '샘플상세자료에 파일이 첨부되지 않았습니다.');
			}else{
				form.findField('smp_status').setValue('SMSTAT_050');
				form.findField('smp_status_nm').setValue('요청접수');  
				form.findField('rruser_nm').setValue(username);
				form.findField('rruser_cd').setValue(loginUser);
				form.findField('smr_rrdate').setValue(new Date());
				this.dbProc();
			}
	},
	onModify: function () {
	    var pl = Ext.getCmp('receiptDetail');
	    var form = Ext.getCmp('receiptDetail').getForm();  
	    Ysn.Util.cbEmptyVal(Ext.getCmp('receiptDetail')); 
	    if (form.isValid()) {
		...
		if (form.isValid()) {
			...
			form.submit({
				...
				success: function(form, action) {
				   ...
				   pl.down('#muiltFileBox1').down('#paentFrm').setValue('samplerequest-receipt');
					pl.down('#muiltFileBox1').down('#childFrm').setValue('#muiltFile1');
					pl.down('#muiltFileBox2').down('#paentFrm').setValue('samplerequest-receipt');
