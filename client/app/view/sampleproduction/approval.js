Ext.define('Ysn.view.sampleproduction.approvalsearch',{
    extend: 'Ext.panel.Panel',
	xtype: 'approval-search',

    requires: [
        'Ysn.view.sampleproduction.approvalsearchController', 
	  	'Ysn.view.common.searchcustomer',
	    'Ysn.store.*'
    ],

    controller: 'approval-search', 
    ...
    items: [{
            ...
            if(!Ysn.Util.OnsessOut(operation._response.responseText)) return false; 
												...
												var bizCd = Ext.getCmp('approval-search').down('#bizGroup');
												if(store.data.items.length < 2){
													Ext.getCmp('approval-search').down('#user_cd').store.load({params:{dept_cd:bizCd.getValue(),up_dept_cd:''}});
												}else{
													Ext.getCmp('approval-search').down('#user_cd').store.load({params:{dept_cd:'',up_dept_cd:bizCd.getValue()}});
												}
									   }}
						},
						minChars: 0,
				...
				defaults: {
					...
					items: [{
						fieldLabel: Locale.getMsg('제작의뢰일'),
						labelAlign: 'right',
						labelWidth: 80,
						width: 210,
						xtype: 'datefield',
           ...
           items: [{
               ...
               xtype: 'combobox',
               reference: 'prdt_auser',
               publishes: 'value',
               fieldLabel: Locale.getMsg('승인담당'),
               labelAlign: 'right',
               displayField: 'USER_NM',
               valueField: 'USER_CD',
               name: 'prdt_auser',
...
Ext.define('Ysn.view.sampleproduction.approval',{
    extend: 'Ext.panel.Panel',
    xtype: 'sampleproduction-approval',
    requires: [
        'Ysn.view.sampleproduction.approvalController', 
		'Ysn.view.common.searchcustomer'
    ],

    controller: 'sampleproduction-approval',
    reference: 'sampleproduction-approval',
	id:'sampleproduction-approval',
    layout: 'border',
    width: 500,
    height: 400,
    overflow:'hidden',
