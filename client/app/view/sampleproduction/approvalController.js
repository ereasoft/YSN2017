...
Ext.define('Ysn.view.sampleproduction.approvalController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.sampleproduction-approval',
	requires: [ 
	    'Ysn.store.*'
    ],
    init: function() {
        this.lookupReference('approvalSearch').setTitle(pageTitle);
		if(Ext.getCmp('chkpopup').getValue()){ 
			Ext.getCmp('sampleproduction-approval').down('#east').setVisible(false);			
		} 
		Ext.getCmp('approvalDetail').down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('approvalDetail').down('#muiltFileBox1').down('#AttachBtn').hide();

		Ext.getCmp('approvalDetail').down('#muiltFileBox2').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('approvalDetail').down('#muiltFileBox2').down('#AttachBtn').hide();

		Ext.getCmp('approvalDetail').down('#muiltFileBox3').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('approvalDetail').down('#muiltFileBox3').down('#AttachBtn').hide();
	},
	itemclick: function(view, record, index, e) {  
		if(Ext.getCmp('chkpopup').getValue()){
            openPopupView('샘플제작승인','sampleproduction-approvalDetail',record.get('SMP_CD'),'SampleManage|selectSampleDetail|smp_cd|smp_chasu|'+record.get('SMP_CHASU')); 
		}else{
			//if (Ext.getCmp('sampleproduction-approval').down('#east').collapsed) Ext.getCmp('sampleproduction-approval').down('#east').toggleCollapse(); 
			...
			Pl.load({
				...
				success: function(form, action) {
				    ...
				    var Pl = Ext.getCmp('approvalDetail');
				    if (record.get('SMP_STATUS') == 'SMSTAT_200') {
				        if (record.get('SMP_TYPE') != 'SMPTP_400') {
				            Pl.down('#btn1').show();
				            Pl.down('#btn2').show();
				            Pl.down('#btn3').hide();
				        } else {
				    ...
				    } else if (record.get('SMP_STATUS') == 'SMSTAT_400') {
				        Pl.down('#btn1').hide();
				        Pl.down('#btn2').hide();
				        Pl.down('#btn3').show();
		...
		var task = new Ext.util.DelayedTask(function () {
		    Ext.getCmp('sampleproduction-approval').down('#east').toggleCollapse();
		});
		if (Ext.getCmp('sampleproduction-approval').down('#east').collapsed) task.delay(1000);
	...
	frmClear: function(){
		Ext.getCmp('approvalDetail').getForm().reset(); 
		Ext.getCmp('approvalDetail').getForm().findField('addchk').setValue('add');
		Ext.getCmp('approvalDetail').down('#east').toggleCollapse();
	},
	xlsExport: function(){  

		Ext.getCmp('approvalList').saveDocumentAs({ 
			headerRowCnt: 1,
            type:       'xlsx',
            title:      Ext.getCmp('approval-search').getTitle(),
            fileName:   '샘플제작승인.xlsx' 
		});
	},
	
	batchCommit: function () {
	    var selRecs = Ext.getCmp('approvalList').getSelectionModel().getSelection();
	    var prdt_adate = Ext.Date.format(new Date(), 'Y-m-d');
	    var prdt_auser = loginUser;
	    var muser_cd = '';
	    var muser_cd2 = '';
	    var smp_cd = '';
	    var smp_chasu = '';
	    ...
	    var smp_qty = '';
	    var flag = false;
	    if (Ext.getCmp('approval-search').lookupReference('smp_type').getValue() == '') {
	        Ext.Msg.alert('Warning', '샘플유형을 먼저 선택하세요. ');
	        return false;
	    }
	    for (var i = 0; i < selRecs.length; i++) {
	        if (selRecs[i].data.SMP_TYPE != 'SMPTP_400') {
	            if (selRecs[i].data.MUSER_NM != null && selRecs[i].data.PRDT_PDATE != null) {
	                muser_cd2 = selRecs[i].data.MUSER_NM;
	                prdt_pdate2 = Ext.Date.format(selRecs[i].data.PRDT_PDATE, 'Y-m-d');
	                flag = true; 
	            }
	        } else {
	            flag = true;
	    ...
	    if (flag == false) {
	        Ext.Msg.alert('Failed', '제작담당과 완료예정일은 필수 선택 입니다. ');
	        return false;
	    }
	    ...
	    for (var i = 0; i < selRecs.length; i++) {
	        ...
	        muser_cd = muser_cd + muser_cd2 + split;
	        prdt_pdate = prdt_pdate + prdt_pdate2 + split;
	        smp_type = smp_type + selRecs[i].data.SMP_TYPE + split;
	        smp_qty = smp_qty + "0" + split;
	    }
	    ...
	    Ext.Ajax.request({
	        ...
	        method: 'POST',
	        params: {
	            smp_cd: smp_cd, smp_chasu: smp_chasu, smp_qty: smp_qty, muser_cd: muser_cd, prdt_pdate: prdt_pdate, smp_type: smp_type, state: 'COMMITION_SUCCESS'
	        },
	        success: function (action) {
				if(!Ysn.Util.OnsessOut(action.responseText)) return false;
	            Ext.Msg.alert(Locale.getMsg('처리상태'), '일괄제작승인완료');
	            Ext.getCmp('approvalList').getStore().reload();
	            Ext.getCmp('approvalDetail').getForm().reset();
	        },
...
Ext.define('Ysn.view.sampleproduction.approvalsearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.approval-search',
	init: function() {
	 this.lookupReference('bizGroup').store.load();  
	 this.lookupReference('bizGroup').setValue(Ysn.Util.chkDept(true));
	 this.lookupReference('prdt_auser').store.load();  
     this.lookupReference('prdt_auser').setValue('');  
	...
	openWindow: function(){
       var win = Ext.getCmp('commonSearchcustomer');
		...
		hidfield.setValue('approval-search');
		Ext.getCmp('sampleproduction-approval').add(win);
	...
	onSubmitClick: function() {
		...
		var Frm = Pl.getForm(); 
	    Frm.reset();
	    //Pl.down('#muiltFileBox1').down('#paentFrm').setValue('sampleproduction-approval');
	    ...
	    Ext.getCmp('approvalList').getStore().load(
					{params: {  deptGroup : this.lookupReference('deptGroup').getValue(),
								...
								prdt_auser   : this.lookupReference('prdt_auser').getValue(),								
								...
								s_prdt_rdate: Ext.Date.format(this.lookupReference('sdate').getValue(), 'Y-m-d'),
								e_prdt_rdate: Ext.Date.format(this.lookupReference('edate').getValue(), 'Y-m-d'),
				                mode: 'B'
		            }}
		);
	}
