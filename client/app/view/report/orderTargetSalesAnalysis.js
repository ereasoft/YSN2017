    Ext.define('Ysn.view.report.orderTargetSalessearch', {
        extend: 'Ext.panel.Panel',
        xtype: 'orderTargetSales-search',

        requires: [
            'Ysn.view.report.orderTargetSalessearchController', 
	        'Ysn.store.*'
        ],

        controller: 'orderTargetSales-search',
        ...
        id: 'orderTargetSales-search',
        reference: 'orderTargetSales-search',
        defaults: {
            layout: 'form',
            xtype: 'container',
                    ...
                    store: {
                        ...
                        listeners: {
                            load: function (store) {
                                ...
                                var bizCd = Ext.getCmp('orderTargetSales-search').down('#bizGroup');
                                if (store.data.items.length < 2) {
                                    Ext.getCmp('orderTargetSales-search').down('#user_cd').store.load({ params: { dept_cd: bizCd.getValue(), up_dept_cd: '' } });
                                } else {
                                    Ext.getCmp('orderTargetSales-search').down('#user_cd').store.load({ params: { dept_cd: '', up_dept_cd: bizCd.getValue() } });
                                }
                            }
                        }
                    },
    ...
    Ext.define('Ysn.view.report.orderTargetSalessearchController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.orderTargetSales-search',
        init: function () {
        ...
        openWindow: function () {
            ...
            hidfield.setValue('orderTargetSales-search');
		    win.down('#cust_nm').setValue(this.lookupReference('cust_nm').getValue());
            Ext.getCmp('report-orderTargetSales').add(win);
        ...
        openWindow2: function () {
            ...
            hidfield.setValue('orderTargetSales-search');
            Ext.getCmp('report-orderTargetSales').add(win2);
        ...
        openWindow3: function () {
            ...
            hidfield.setValue('orderTargetSales-search');
            Ext.getCmp('report-orderTargetSales').add(win3);
        ...
        onSubmitClick: function () {
            var pl = Ext.getCmp('orderTargetSalesList');
            ...
            pl.getStore().load(
                    {
                        ...
                        callback: function (records, operation, success) {
                           // pl.removeAll();
                            var col = pl.getController('report-orderTargetSalesList').changeColumns(operation._params.base_yy, parseInt(operation._params.base_mm1), parseInt(operation._params.base_mm2)); 
                            //pl.getColumnManager().headerCt.removeAll();
                            pl.reconfigure(col);
                            //pl.updateLayout();
    ...
    Ext.define('Ysn.view.report.orderTargetSalesList', {
        extend: 'Ext.grid.Panel',
        xtype: 'orderTargetSalesList',
        requires: [
            'Ysn.view.report.orderTargetSalesListController', 
		    'Ysn.store.orderTargetSalesList'
        ],
    
        controller: 'report-orderTargetSalesList',     
	    store: {
	        type: 'orderTargetSalesList',
		    autoLoad: false,
            autoDestroy: true
        }, 
    ...
    Ext.define('Ysn.view.report.orderTargetSalesListController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.report-orderTargetSalesList',
        init: function () {
            var Today = new Date();            
    ...
    Ext.define('Ysn.view.report.orderTargetSales', {
        extend: 'Ext.panel.Panel',
        xtype: 'report-orderTargetSales',
        requires: [
            'Ysn.view.report.orderTargetSalesController',
		    'Ysn.view.common.searchcustomer',
            'Ysn.view.report.orderTargetSalessearch',
            'Ysn.view.report.orderTargetSalesList'
        ],

        controller: 'report-orderTargetSales',
        reference: 'report-orderTargetSales',
        id: 'report-orderTargetSales',
        layout: 'border',
        width: 500,
        height: 400,
        overflow: 'hidden',
        ...
        dockedItems: [
	                   {
	                       title: '',
	                       xtype: 'orderTargetSales-search',
	                       reference: 'orderTargetSalesSearch',
	                       itemId: 'orderTargetSalesSearch',
                           scrollable: true,
	                       collapsible: true,
	                       floatable: true,
	                       split: true,
        ...
        items: [
		    {
		        ...
		        region: 'center',
		        xtype: 'orderTargetSalesList',
		        reference: 'orderTargetSalesList',
		        itemId: 'orderTargetSalesList',
		        listeners: {
		            //select: 'itemclick'
		        }
    ...
    Ext.define('Ysn.view.report.orderTargetSalesController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.report-orderTargetSales',
        requires: [
	        'Ysn.store.*'
        ],
        init: function () {
            this.lookupReference('orderTargetSalesSearch').setTitle(pageTitle); 
        },
       
        xlsExport: function () {

            Ext.getCmp('orderTargetSalesList').saveDocumentAs({
			headerRowCnt: 3,
                type: 'xlsx',
                title: Ext.getCmp('orderTargetSales-search').getTitle(),
                fileName: '수주목표Vs실적분석'
            });
        }
    });
