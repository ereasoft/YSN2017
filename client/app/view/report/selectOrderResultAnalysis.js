    Ext.define('Ysn.view.report.selectOrderResultsearch', {
        extend: 'Ext.panel.Panel',
        xtype: 'selectOrderResult-search',

        requires: [
            'Ysn.view.report.selectOrderResultsearchController', 
	        'Ysn.store.*'
        ],

        controller: 'selectOrderResult-search',
        ...
        id: 'selectOrderResult-search',
        reference: 'selectOrderResult-search',
        defaults: {
            layout: 'form',
            xtype: 'container',
                    ...
                    store: {
                        ...
                        listeners: {
                            load: function (store) {
                                ...
                                var bizCd = Ext.getCmp('selectOrderResult-search').down('#bizGroup');
                                if (store.data.items.length < 2) {
                                    Ext.getCmp('selectOrderResult-search').down('#user_cd').store.load({ params: { dept_cd: bizCd.getValue(), up_dept_cd: '' } });
                                } else {
                                    Ext.getCmp('selectOrderResult-search').down('#user_cd').store.load({ params: { dept_cd: '', up_dept_cd: bizCd.getValue() } });
                                }
                            }
                        }
                    },
    ...
    Ext.define('Ysn.view.report.selectOrderResultsearchController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.selectOrderResult-search',
        init: function () {
        ...
        openWindow: function () {
            ...
            hidfield.setValue('selectOrderResult-search');
		    win.down('#cust_nm').setValue(this.lookupReference('cust_nm').getValue());
            Ext.getCmp('report-selectOrderResult').add(win);
        ...
        openWindow2: function () {
            ...
            hidfield.setValue('selectOrderResult-search');
            Ext.getCmp('report-selectOrderResult').add(win2);
        ...
        openWindow3: function () {
            ...
            hidfield.setValue('selectOrderResult-search');
            Ext.getCmp('report-selectOrderResult').add(win3);
        ...
        onSubmitClick: function () {
            var pl = Ext.getCmp('selectOrderResultList');
    ...
    Ext.define('Ysn.view.report.selectOrderResultList', {
        extend: 'Ext.grid.Panel',
        xtype: 'selectOrderResultList',
        requires: [
            'Ysn.view.report.selectOrderResultListController', 
		    'Ysn.store.selectOrderResultList'
        ],
    
        controller: 'report-selectOrderResultList',     
	    store: {
	        type: 'selectOrderResultList',
		    autoLoad: false,
            autoDestroy: true
        }, 
    ...
    Ext.define('Ysn.view.report.selectOrderResultListController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.report-selectOrderResultList',
        init: function () {
            var Today = new Date();            
    ...
    Ext.define('Ysn.view.report.selectOrderResult', {
        extend: 'Ext.panel.Panel',
        xtype: 'report-selectOrderResult',
        requires: [
            'Ysn.view.report.selectOrderResultController',
		    'Ysn.view.common.searchcustomer',
            'Ysn.view.report.selectOrderResultsearch',
            'Ysn.view.report.selectOrderResultList'
        ],

        controller: 'report-selectOrderResult',
        reference: 'report-selectOrderResult',
        id: 'report-selectOrderResult',
        layout: 'border',
        width: 500,
        height: 400,
        overflow: 'hidden',
        ...
        dockedItems: [
	                   {
	                       title: '',
	                       xtype: 'selectOrderResult-search',
	                       reference: 'selectOrderResultSearch',
	                       itemId: 'selectOrderResultSearch',
                           scrollable: true,
	                       collapsible: true,
	                       floatable: true,
	                       split: true,
        ...
        tbar: {
            ...
            items: [
            {
                ...
                items: [{ text: '월별수주실적', pressed: true },
                        { text: '분기별수주실적' } 
                ],
                listeners: {
                    toggle: 'btnToggle'
        ...
        items: [
		    {
		        ...
		        region: 'center',
		        xtype: 'selectOrderResultList',
		        reference: 'selectOrderResultList',
		        itemId: 'selectOrderResultList',
		        listeners: {
		            //select: 'itemclick'
		        }
    ...
    Ext.define('Ysn.view.report.selectOrderResultController', {
        extend: 'Ext.app.ViewController',
        alias: 'controller.report-selectOrderResult',
        requires: [
	        'Ysn.store.*'
        ],
        init: function () {
            this.lookupReference('selectOrderResultSearch').setTitle(pageTitle); 
        },
        btnToggle: function (container, button, pressed) {
            var pl = this.getView().lookupReference('selectOrderResultSearch');
            var p2 = this.getView().lookupReference('selectOrderResultList');
            var col, store;
            switch (button.getText()) {
                case "월별수주실적":
                    ...
                    col = p2.getController('report-selectOrderResultList').changeColumns(pl.lookupReference('base_yy').getValue(), parseInt(pl.lookupReference('base_mm1').getValue()), parseInt(pl.lookupReference('base_mm2').getValue()), '1');
                    store = 'selectOrderResultList';
                    break;
                case "분기별수주실적":
                    ...
                    col = p2.getController('report-selectOrderResultList').changeColumns(pl.lookupReference('base_yy').getValue(), parseInt(pl.lookupReference('base_div1').getValue()), parseInt(pl.lookupReference('base_div2').getValue()), '2');
        ...
        xlsExport: function () {
            ...
            Ext.getCmp('selectOrderResultList').saveDocumentAs({
			headerRowCnt: 2,
                type: 'xlsx',
                title: Ext.getCmp('selectOrderResult-search').getTitle(),
                fileName: '수주실적분석'
            });
        }
    });
