...
Ext.define('Ysn.view.productinquiry.followupList',{
    extend: 'Ext.grid.Panel',
    xtype: 'followupList',
    requires: [
        'Ysn.view.productinquiry.followupListController', 
		'Ysn.store.customerList',
		'Ext.grid.filters.Filters' 
    ],
    
    controller: 'productinquiry-followupList',     
	store: {
        type: 'followupList', 
		autoLoad: false,
        autoDestroy: true
	...
	columns: [
            ...
            {text: Locale.getMsg('문의경로'),  width: 125, dataIndex: 'INQ_CHNL_NM', sortable: true,   
			 filter: {
				type: 'string',
				itemDefaults: {
			...
			{
			    text: Locale.getMsg('국가'), width: 125, dataIndex: 'NAT_NM', sortable: true,
			    filter: {
			        type: 'string',
			        itemDefaults: {
			            emptyText: Locale.getMsg('검색어입력..')
            ...
            {text: Locale.getMsg('조치내역'),flex:1, dataIndex: 'POST_ARNG', sortable: true, 
			 filter: {
				 type: 'string',
				 itemDefaults: {
					emptyText: Locale.getMsg('검색어입력..')
...
Ext.define('Ysn.view.productinquiry.followupDetail',{
    extend: 'Ext.form.Panel',
    xtype: 'productinquiry-followupDetail',
    requires: [
        'Ysn.view.productinquiry.followupDetailController'
    ],

    controller: 'productinquiry-followupDetail', 


    frame: true,
    id: 'followupDetail',
	reference: 'followupDetail',
    bodyPadding: 10,
    scrollable:true,
    width: 800,   
    reader: {
    ...
    items: [{
        ...
        items: [                       
					...
					{ fieldLabel: Locale.getMsg('사후조치내용'), xtype: 'textareafield', labelWidth: 100, style: { width: '100%' }, name: 'post_arng', rowspan: 2},
					{ fieldLabel: Locale.getMsg('매출발생여부'), xtype: 'checkboxfield', labelWidth: 100, inputValue: 'Y', name: 'order_yn' },
					{ fieldLabel: Locale.getMsg('매출금액'), minValue: 0, xtype: 'numberfield', allowDecimals: false, labelWidth: 100, style: { width: '100%' }, name: 'order_amount' },
					{ xtype: 'hiddenfield', name: 'addchk', value: 'no' },
                    {
        ...
        dockedItems: [{
            ...
            items: [
                ...
                {
                    xtype: 'button', text: Locale.getMsg('접수등록'), margin: '5 5 5 0', itemId: 'btn9', hidden: true,
                    listeners: { click: 'onNew' }
                }, 
                {
                    xtype: 'button', text: Locale.getMsg('담당자재지정'), margin: '5 5 5 0', itemId: 'btn10', hidden: true,
                    listeners: { click: 'onUsrChange' }
                },
                {
                    xtype: 'button', text: Locale.getMsg('매출발생'), margin: '5 5 5 0', itemId: 'btn11', hidden: true,
                    listeners: { click: 'onOrderYn' }
                }, 
                {
                    xtype: 'button', text: Locale.getMsg('수정'), margin: '5 5 5 0', itemId: 'btn12', hidden: false,
