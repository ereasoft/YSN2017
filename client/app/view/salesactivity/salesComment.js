...
Ext.define('Ysn.view.salesactivity.salesComment',{
    extend: 'Ext.panel.Panel',
    xtype: 'salesactivity-salesComment',
    requires: [
        'Ysn.view.salesactivity.salesCommentController', 
		'Ysn.view.salesactivity.salesCommentList'
    ],

    controller: 'salesactivity-salesComment', 
    reference: 'salesactivitysalesComment',
	id:'salesactivitysalesComment',
    layout: 'border',
    width: 500,
    height: 400,
    overflow:'hidden',
	...
	dockedItems: [
	               {
		            title: Locale.getMsg('영업활동코멘트관리'),
					xtype: 'salesComment-search',
					reference: 'salesCommentSearch',
                           scrollable: true,
					collapsible: true,
					floatable: true,
					split: true,
    ...
    items: [
		{
					header: false,					
					region: 'center',					
					xtype: 'salesCommentList',
					itemId: 'salesCommentList',
                    listeners:{
                               select:'itemclick'
                    }
                    ...
                    maxWidth: 1200,
					items: {
						xtype: 'salesactivity-salesCommentDetail',
						itemId: 'salesactivity-salesCommentDetail'
					}
				}
    ]
...
Ext.define('Ysn.view.salesactivity.salesCommentController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.salesactivity-salesComment',
	requires: [ 
	    'Ysn.store.*'
    ],
    init: function() {
        this.lookupReference('salesCommentSearch').setTitle(pageTitle);
		Ext.getCmp('salesCommentDetail').down('#muiltFileBox1').down('#AttachFileList').down('#delbtn').hide();
		Ext.getCmp('salesCommentDetail').down('#muiltFileBox1').down('#AttachBtn').hide();
		if(Ext.getCmp('chkpopup').getValue()){ 
			Ext.getCmp('salesactivitysalesComment').down('#east').setVisible(false);			
		} 
	},
	itemclick: function(view, record, index, e) { 
		if(Ext.getCmp('chkpopup').getValue()){
            openPopupView(Locale.getMsg('영업활동코멘트관리'),'salesactivity-salesCommentDetail',record.get('SA_CD'),'SalesActivity|popupSalesActivityDetail|sa_cd'); 
		}else{
			if (Ext.getCmp('salesactivitysalesComment').down('#east').collapsed) Ext.getCmp('salesactivitysalesComment').down('#east').toggleCollapse(); 
			var Pl = Ext.getCmp('salesCommentDetail');
			Pl.load({
	...
	onClick: function(){  
		...
		Ext.getCmp('salesCommentList').saveDocumentAs({ 
			headerRowCnt: 1,
            type:       'xlsx',
            title:      Ext.getCmp('salesComment-search').getTitle(),
            fileName:   Locale.getMsg('영업활동코멘트관리')
		});
	}
});

Ext.define('Ysn.view.salesactivity.salesCommentsearch',{
    extend: 'Ext.panel.Panel',
	xtype: 'salesComment-search',

    requires: [
        'Ysn.view.salesactivity.salesCommentsearchController', 
	    'Ysn.store.*'
    ],

    controller: 'salesComment-search',
					...
					items: [{
						...
						store: {
							...
							listeners: { load: function (store, records, successful, operation) {
												...
												var bizCd = Ext.getCmp('salesComment-search').down('#bizGroup');
												if(store.data.items.length < 2){
													Ext.getCmp('salesComment-search').down('user_cd').store.load({params:{dept_cd:bizCd.getValue(),up_dept_cd:''}});
												}else{
													Ext.getCmp('salesComment-search').down('user_cd').store.load({params:{dept_cd:'',up_dept_cd:bizCd.getValue()}});
												}
									   }}
						},
						minChars: 0,
...
Ext.define('Ysn.view.salesactivity.salesCommentsearchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.salesComment-search',
	init: function() {
	...
	onSubmitClick: function() {
			...
			if (deptGrp.getStore().data.items.length < 2 )
			{
				deptCode = bizCode;
				bizCode = '';
			}//'salesactivity-salesCommentDetail'
            Ext.getCmp('salesactivitysalesComment').down('#salesactivity-salesCommentDetail').getForm().reset();
            Ext.getCmp('salesCommentDetail').down('#muiltFileBox1').down('#AttachFileList').getStore().removeAll();
			Ext.getCmp('salesactivitysalesComment').down('#salesCommentList').store.load(
...
Ext.define('Ysn.view.salesactivity.salesCommentList',{
    extend: 'Ext.grid.Panel',
    xtype: 'salesCommentList',
    requires: [
        'Ysn.view.salesactivity.salesCommentListController', 
		'Ysn.store.salesCommentList',
		'Ext.grid.filters.Filters'
    ],
    
    controller: 'salesactivity-salesCommentList',     
	store: {
        type: 'salesCommentList', 
		autoLoad: false,
        autoDestroy: true
    }, 
	...
	columns: [
			...
			{text: Locale.getMsg('주요활동'), width: 125, dataIndex: 'SA_TARGET', sortable: true,  
			 filter: {
				 type: 'string',
				 itemDefaults: {
					emptyText: Locale.getMsg('검색어입력..')
		    ...
		    {text: Locale.getMsg('활동결과'), width: 250, dataIndex: 'SA_BODY', sortable: true,  
			 filter: {
				 type: 'string',
				 itemDefaults: {
					emptyText: Locale.getMsg('검색어입력..')
            ...
            {text: Locale.getMsg('코멘트내역'), width: 250, dataIndex: 'SA_COMMENT', sortable: true, 
			 filter: {
				 type: 'string',
				 itemDefaults: {
					emptyText: Locale.getMsg('검색어입력..')
...
Ext.define('Ysn.view.salesactivity.salesCommentListController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.salesactivity-salesCommentList', 
    init: function() {
...
Ext.define('Ysn.view.salesactivity.salesCommentDetail',{
    extend: 'Ext.form.Panel',
    xtype: 'salesactivity-salesCommentDetail',
    requires: [
        'Ysn.view.salesactivity.salesCommentDetailController'
    ],

    controller: 'salesactivity-salesCommentDetail', 


    frame: true,
    id: 'salesCommentDetail',
	reference: 'salesCommentDetail',
    bodyPadding: 10,
    scrollable:true,
    width: 700,   
    reader: {
...
Ext.define('Ysn.view.salesactivity.salesCommentDetailController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.salesactivity-salesCommentDetail', 
    init: function() {
	 this.lookupReference('sa_type').store.load({params:{up_code_id:'SA_TYPE', lang: localeCd}}); 
	 this.lookupReference('sa_cnct').store.load({params:{up_code_id:'SA_CNCT', lang: localeCd}});   
	},
	onSubmit: function(){ 
		var form = Ext.getCmp('salesCommentDetail').getForm(); 
		Ysn.Util.cbEmptyVal(Ext.getCmp('salesCommentDetail')); 
		if (form.isValid()) {
			form.submit({
				waitMsg:'Processing...',
                url: '/salesComment/popupSalesActivityDetail',
                method: 'POST',
            ...
            Ysn.Util.OnsessOut(action.response.responseText);
                   ...
                   //Ext.getCmp('salesCommentList').store.reload();
				},
	...
	keyFactorVal : function(el , newValue , oldValue , eOpts){
		if(newValue != ''){
			...
			for(var i in sa_targets){
				Ext.getCmp('salesCommentDetail').down('#'+sa_targets[i]).setValue(sa_targets[i]);
			}
		}
	}
